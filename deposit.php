...
// Récupérer le solde DIRECTEMENT depuis la BDD pour être sûr
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($amount <= 0) {
        ...
        $result = $transactionManager->deposit($userId, $amount, $method, $notes);
...
<body>
    <div class="app-container max-w-md mx-auto min-h-screen flex flex-col">
        ...
        <header class="glass p-4 flex justify-between items-center z-10">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-lg font-bold text-[var(--text-primary)]">💰 Dépôt</h1>
            </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 pb-24">
            ...
            <!-- AFFICHER LE SOLDE DIRECTEMENT DEPUIS LA BDD -->
            ...
            <div class="glass p-6 rounded-2xl">
                <h2 class="text-sm font-bold text-[var(--text-primary)] mb-4">📥 Ajouter des fonds</h2>
                
                <form method="POST" class="space-y-4" id="depositForm">
                    <div>
                        <label class="text-sm text-[var(--text-secondary)] block mb-1">Montant (Ar)</label>
                        <input type="number" name="amount" id="depositAmount" required min="100" step="100" 
                               class="input-modern" placeholder="1000" 
                               value="<?php echo htmlspecialchars($_POST['amount'] ?? ''); ?>">
                        <p class="text-xs text-[var(--text-secondary)] mt-1">Minimum: 100 Ar</p>
                    </div>
                    ...
                    <button type="submit" class="btn-primary w-full py-3 text-base">
                        💰 Déposer <?php echo formatCurrency($_POST['amount'] ?? 0); ?>
                    </button>
                </form>
                
                <div class="mt-4 p-3 glass rounded-lg text-xs text-[var(--text-secondary)]">
                    <p>💡 Les dépôts sont immédiats et sécurisés.</p>
    ...
    <script>
        ...
        document.getElementById('depositAmount')?.addEventListener('input', function() {
            ...
            if (btn) {
                btn.textContent = `💰 Déposer ${formatCurrency(amount)}`;
            }
        });
